...
aug = ImgAug()      # 데이터 증강 class 선언
augment_num = 10    # 증강 결과로 출력되는 이미지의 개수 선언
train_path = './data/images/train/fish'
...
for folder in folder_list:
    ...
    for file in file_list:
        ...
        for num, aug_img in enumerate(images_aug):
            cv2.imwrite(folder_path + '/' + file.split('.')[0] + '_' + str(num+1) + '.' + file.split('.')[1], aug_img)

    print('* ' + folder + " *" + ' Complete augmenting images')
